...
const navItems: NavItem[] = [
  ...
  {
    path: '/about', label: 'About', icon: <IconUser size={16} />,
    sub: ['• Biography', '• Qualifications', '• With Rev. K. K. Sam']
  },
  {
    path: '/counselling', label: 'Counselling', icon: <IconHeartHandshake size={16} />,
    sub: ['• Individual', '• Family & Marriage', "• Youth & Women's", '• Pre-marital', '• Emotional Wellness', '• Addiction', '• Group Therapy']
  },
  {
    path: '/bible-teaching', label: 'Bible Teaching', icon: <IconBook2 size={16} />,
    sub: ['• Sermons & Messages', '• Bible Colleges', "• Women's Seminars"]
  },
  { path: '/courses', label: 'Courses', icon: <IconCertificate size={16} /> },
  { path: '/media', label: 'Media Ministry', icon: <IconDeviceTv size={16} /> },
  { path: '/gallery', label: 'Gallery', icon: <IconPhoto size={16} /> },
  { path: '/testimonials', label: 'Testimonials', icon: <IconQuote size={16} /> },
    ...
    <div style={{
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px', justifyContent: 'center', marginBottom: '10px' }}>
            {['Counselor', 'Bible Teacher', 'Speaker', 'Life Skills Trainer'].map((tag: string) => (
              <span key={tag} style={{
                background: 'rgba(187, 149, 87, 0.65)',
                color: 'var(--gold)',
                fontSize: '8px',
                padding: '2px 6px',
                borderRadius: '10px',
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '8px', display: 'flex', flexDirection: 'column', gap: '5px', textAlign: 'left' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
                ...
                <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '9.5px', lineHeight: 1.5 }}>{item.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '8px 6px' }}>
          <div style={{ fontSize: '8px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.25)', padding: '8px 8px 4px' }}>
            Main Menu
          </div>
            ...
            return (
              <div key={item.path}>
                <Link to={item.path} style={{
                  ...
                  <span style={{ fontSize: '12px', color: isActive ? 'var(--gold)' : 'rgba(255,255,255,0.65)', fontWeight: isActive ? 500 : 400, flex: 1 }}>
                    {item.label}
                  </span>
                ...
                {item.sub && isActive && (
                  <div style={{ padding: '0 6px 4px 28px' }}>
                    {item.sub.map((s: string) => (
                      <div key={s} style={{ padding: '3px 6px', fontSize: '10px', color: 'rgba(255,255,255,0.4)' }}>
                         {s}
                      </div>
                    ))}
                  </div>
          ...
          {/* QUICK CONTACT */}
          <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '8px', margin: '0 4px 4px', padding: '11px 10px' }}>
            <div style={{ color: 'var(--gold)', fontSize: '8.5px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '8px' }}>
              Quick Contact
            </div>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'flex-start', marginBottom: '6px' }}>
                <span style={{ color: 'var(--gold)', marginTop: '1px', flexShrink: 0 }}>{item.icon}</span>
                <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: '9.5px', lineHeight: 1.5, whiteSpace: 'pre-line' }}>{item.text}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: '7px', padding: '8px 10px 12px' }}>
            {[
              ...
              { icon: <IconBrandWhatsappFilled size={14} />, href: '#' },
